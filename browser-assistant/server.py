...
class _Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # mode dispatch: stealth → camofox anti-detection Firefox (ADR MED-012);
            # headless/comet → browser-use Chrome. run_stealth_task takes the same kwargs
            # (cdp_url/use_vision ignored) so the call sites below are identical.
            if mode == "stealth":
                from camofox_agent import run_stealth_task as _runner
            else:
                from agent_runner import run_task as _runner
