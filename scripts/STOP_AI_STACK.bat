...
REM ── 2c. Sentinel Bridge (port 8097) ──────────────
echo [2c] Stopping Sentinel Bridge...
for /f "tokens=5" %%a in ('netstat -ano 2^>nul ^| findstr ":8097 " ^| findstr "LISTENING"') do (
    taskkill /PID %%a /F >nul 2>&1
)
echo        OK
...
REM ── 4b. smdl container (separate compose file) ──
echo [4b] Stopping smdl container...
cd /d C:\Users\azfar\metamcp-local
docker compose -f docker-compose.smdl.yml down >nul 2>&1
echo        OK

REM ── 4c. Firefly III ───────────────────────────────
echo [4c] Stopping Firefly III...
cd /d C:\Users\azfar\metamcp-local
docker compose -f docker-compose.firefly.yml down >nul 2>&1
echo        OK
