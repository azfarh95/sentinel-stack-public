...
# Writes Netscape-format cookies.txt files to the configured cookies dir
# which is bind-mounted into the ytdlp-mcp container at /cookies
#
# Default location: %USERPROFILE%\YT-DLP\cookies — override via:
#   $env:SENTINEL_COOKIES_DIR = "D:\some\other\path"  (for the current shell)
# or set the env var system-wide.

if ($env:SENTINEL_COOKIES_DIR) {
    $CookieDir = $env:SENTINEL_COOKIES_DIR
} elseif (Test-Path "G:\YT-DLP\cookies") {
    # Backwards-compat for current owner setup
    $CookieDir = "G:\YT-DLP\cookies"
} else {
    $CookieDir = Join-Path $env:USERPROFILE "YT-DLP\cookies"
}
