...
function Start-PlaywrightMCP {
    if (Get-PidOnPort $PlaywrightPort) {
        ...
        Start-Process -FilePath "cmd.exe" -ArgumentList "/c `"$NpxPath`" -y @playwright/mcp@latest --port $PlaywrightPort --browser chrome --allowed-hosts * --shared-browser-context" -WindowStyle Hidden
