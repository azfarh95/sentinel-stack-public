...
@mcp.tool()
async def gmail_list_labels() -> list:
    """List all Gmail labels (system + user) with id, name, and type."""
    try:
        return await gmail_tools.list_labels()
    except Exception as e:
        return [{"error": str(e)}]
...
async def gmail_create_label(name: str) -> dict:
    """
    Create a Gmail label. Idempotent — returns the existing label if name matches.
    Use '/' for nested labels, e.g. 'Bank Statements/HSBC Statement'.
    """
    try:
        return await gmail_tools.create_label(name)
    except Exception as e:
        return {"error": str(e)}


@mcp.tool()
async def gmail_apply_label(email_id: str, label: str) -> dict:
    """
    Add a label to an email. `label` accepts a label_id or a label_name
    (e.g. 'Bank Statements/HSBC Statement' — name will be resolved to id).
    """
    try:
        return await gmail_tools.apply_label(email_id, label)
    except Exception as e:
        return {"error": str(e)}


@mcp.tool()
async def gmail_remove_label(email_id: str, label: str) -> dict:
    """Remove a label from an email. `label` accepts label_id or label_name."""
    try:
        return await gmail_tools.remove_label(email_id, label)
    except Exception as e:
        return {"error": str(e)}


@mcp.tool()
async def gmail_delete_label(label: str) -> dict:
    """Delete a user label (system labels cannot be deleted). Accepts id or name."""
    try:
        return await gmail_tools.delete_label(label)
    except Exception as e:
        return {"error": str(e)}


@mcp.tool()
async def gmail_list_filters() -> list:
    """List all Gmail filter rules with their criteria and actions."""
    try:
        return await gmail_tools.list_filters()
    except Exception as e:
        return [{"error": str(e)}]
...
async def gmail_create_filter(
    ...
    """
    Create a Gmail filter rule that auto-applies labels to incoming mail.
    add_labels / remove_labels: list of label_ids OR label_names.
    Criteria (provide at least one):
      from_addr: From header (e.g. 'ebanking@mail.hsbc.com.sg')
      query: Gmail search syntax for advanced rules (e.g. 'from:a OR from:b')
      subject / to_addr / has_attachment: simple filters.
    Filters only apply to mail arriving AFTER creation — use gmail_apply_label_to_query
    to backfill existing matching emails.
    """
    try:
        return await gmail_tools.create_filter(
            add_labels, from_addr, to_addr, subject, query, has_attachment, remove_labels
        )
    except Exception as e:
        return {"error": str(e)}
...
@mcp.tool()
async def gmail_delete_filter(filter_id: str) -> dict:
    """Delete a filter rule by ID (use gmail_list_filters to find the ID)."""
    try:
        return await gmail_tools.delete_filter(filter_id)
    except Exception as e:
        return {"error": str(e)}


@mcp.tool()
async def gmail_apply_label_to_query(label: str, query: str, max_results: int = 200) -> dict:
    """
    Retroactively apply a label to all emails matching a Gmail search query.
    Use this right after creating a filter to backfill existing mail.
    label accepts label_id or label_name.
    """
    try:
        return await gmail_tools.apply_label_to_query(label, query, max_results)
    except Exception as e:
        return {"error": str(e)}
