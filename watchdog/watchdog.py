...
OPENCLAW_PAIRING = r"\\wsl.localhost\Ubuntu-24.04\home\azfar\.openclaw\credentials\telegram-pairing.json"
...
OPENCLAW_SESSIONS = r"\\wsl.localhost\Ubuntu-24.04\home\azfar\.openclaw\agents\main\sessions\sessions.json"
...
# Tier 1: HTTP health endpoints (label, url)
HEALTH_ENDPOINTS = [
    ("MetaMCP",        "http://localhost:12008/health"),
    ("Reminders MCP",  "http://localhost:8087/health"),
    ("yt-dlp MCP",     "http://localhost:8088/health"),
    ...
    ("smdl",           "http://localhost:8096/health"),
...
def get_connection_snapshot(cfg: dict) -> dict[str, tuple[bool, str]]:
    ...
    lm_key = cfg.get("lm_studio_api_key")
...
def _restart_main_menu() -> dict:
    return {"inline_keyboard": [
        ...
        [{"text": "↺ smdl  (Nanobot)",               "callback_data": _cb_sign("restart:smdl")}],
...
def _openclaw_ver(cfg: dict) -> str:
    ...
    script = "console.log(JSON.parse(require('fs').readFileSync('/home/azfar/.npm-global/lib/node_modules/openclaw/package.json')).version)"
    try:
        r = subprocess.run(
...
def _run_update(update_id: str, cfg: dict | None = None) -> str:
    ...
    if update_id == "openclaw" and cfg:
        distro = cfg.get("wsl_distro", "Ubuntu-24.04")
        cmd = (
            "npm install -g --prefix /home/azfar/.npm-global openclaw@latest"
