...
# Persist every inference request to a JSONL file so we can audit who's
# calling, when, with what payload, and what got routed where. One line per
# request. Append-only. Rotation handled by external tooling (not size-bounded
# here — single-user owner traffic is light).
...
def _audit_log(record: dict) -> None:
    """Append one JSON line to the audit log. Swallow any error - logging
...
SIMPLE_MODEL  = "qwen/qwen3.6-27b"
COMPLEX_MODEL = "qwen/qwen3.6-27b"
CODING_MODEL  = "qwen/qwen3.6-27b"
# 2026-05-21: pinned all three classes to qwen3.6-27b. Earlier setup routed
# simple→9b / coding→coder-30b but the model swap thrash caused stuck
# PROCESSINGPROMPT states (we saw 9b stuck mid-prompt during a reboot).
# Single resident model = no swap churn. LM Studio keeps qwen3.6-27b in
# memory with Q4_0 K/V cache @ 131072 context. Unload 9b/coder externally
# if VRAM is tight.
        ...
        try:
            ...
            if is_inference and body:
                try:
                    ...
                    # Audit record — capture who/what/when. Hash + first 80 chars
                    # of the last user prompt so we can identify the caller
                    # pattern without leaking full PII.
                    ...
                    audit_record = {"client": self.client_address[0],
                                    "path": self.path,
                                    "user_agent": self.headers.get("User-Agent", ""),
                                    "router_error": str(e)}
            if audit_record is not None:
                _audit_log(audit_record)
