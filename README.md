...
- **MetaMCP tool gateway** — single port aggregates 10+ MCP servers (Memory, Reminders, Maps, GitHub, Google Workspace, OneDrive, yt-dlp, Translate, Playwright, smdl). Tools fan out cleanly, agent picks the right one.
...
| `docker-compose.smdl.yml` | Additive overlay | Always (smdl/Nanobot downloader, runs alongside `.local.yml`) |
...
> **Nanobot (smdl)** — a standalone, non-AI Telegram bot for direct media downloads via yt-dlp and gallery-dl. Lives in its own repo: [YOUR_GITHUB_USERNAME/smdl](https://github.com/YOUR_GITHUB_USERNAME/smdl). Not part of the Sentinel AI stack.
