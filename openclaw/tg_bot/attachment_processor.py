...
import os as _os
# Canonical OCR/vision route since the 2026-06-24 mmproj cutover (ADR AI-016): the
# isolated, CPU-capped vision-mcp service (RapidOCR primary + Tesseract fallback)
# at :8097. The brain is text-only now, so in-model vision (_extract_image_vision
# below) almost always returns None — vision-mcp is the real path.
_VISION_MCP_URL = _os.environ.get("VISION_MCP_URL", "http://127.0.0.1:8097/mcp")


def _run_async(factory, timeout: float = 130.0):
    """Run an async coroutine-factory from sync code, whether or not an event loop
    is already running (the bot's getUpdates path is sync; be defensive anyway)."""
    import asyncio
    try:
        return asyncio.run(asyncio.wait_for(factory(), timeout))
    except RuntimeError:
        import threading
        box: dict = {}
        def _t() -> None:
            ...
            try:
                box["r"] = loop.run_until_complete(asyncio.wait_for(factory(), timeout))
...
def _extract_image_visionmcp(path: Path) -> tuple[str, str] | None:
    """OCR the image via the vision-mcp service (:8097) over MCP streamable-http.
    Returns (header, text) on success, None if vision-mcp is unreachable/errored
    (caller then falls through to in-model vision / tesseract). Passes the image as
    a base64 data: URI so it works regardless of the container's /data mount."""
    try:
        import base64, json as _json
        data = path.read_bytes()
    except Exception as e:
        logger.debug("vision-mcp: can't read %s: %s", path.name, e)
        return None
    ...
    async def _call():
        ...
        async with streamablehttp_client(_VISION_MCP_URL) as (r, w, _):
            async with ClientSession(r, w) as s:
                await s.initialize()
                return await s.call_tool(
                    "read_text", {"source": data_uri, "lang": "en", "engine": "auto"})
    try:
        res = _run_async(_call)
    except Exception as e:  # noqa: BLE001
        logger.warning("vision-mcp read_text failed for %s: %s", path.name, e)
        return None
    if res is None:
        return None
    ...
    if not out:
        try:
            ...
            txt = next((b.text for b in blocks if getattr(b, "type", None) == "text"), None)
            out = _json2.loads(txt) if txt else None
        except Exception:
            out = None
    if not isinstance(out, dict) or not out.get("ok"):
        if isinstance(out, dict) and out.get("error"):
            logger.warning("vision-mcp error for %s: %s", path.name, str(out["error"])[:120])
        return None
    text = (out.get("text") or "").strip()
    eng, ms = out.get("engine", "?"), out.get("ms", "?")
    if not text:
        return (f"Image · vision-mcp ({eng}) found no text",
                "(vision-mcp OCR returned an empty string — likely a non-text image)")
    return (f"Image · vision-mcp ({eng}) · {len(text):,} chars · {ms}ms", _truncate(text))
...
def _extract_image_vision(path: Path) -> tuple[str, str] | None:
    ...
    NOTE: since the 2026-06-24 mmproj cutover the brain is text-only, so this
    almost always returns None now — _extract_image_visionmcp is the live path."""
...
def extract_from_file(path: Path | str, display_name: str | None = None) -> str:
    ...
    if ext == ".pdf":
        ...
        # Extraction ladder (the file IS already downloaded to disk by the bot's
        # _preprocess_attachments — "can't intercept the file" is NOT the failure):
        #   1. vision-mcp :8097 — canonical OCR since the mmproj cutover (RapidOCR
        #      primary, isolated + CPU-capped).
        #   2. in-model VL (legacy; ~always None now the brain is text-only).
        #   3. local tesseract (only if the binary is on THIS host's PATH).
        result = (_extract_image_visionmcp(p)
                  or _extract_image_vision(p)
                  or _extract_image_ocr(p))
        if result is not None:
            header, body = result
        else:
            return (f"[Attached image: {name} · {ext.lstrip('.')} · {size:,} bytes]\n"
                    f"(Image downloaded OK, but no OCR engine answered: vision-mcp "
                    f"(:8097) unreachable AND no local VL model AND tesseract not on "
                    f"this host. Check that the vision-mcp container is running.)")
