...
_INSTAGRAM_STORY_RE = re.compile(
    r"(?i)https?://(?:www\.)?instagram\.com/stories/[\w.-]+/\d+"
)


def _is_instagram_story(url: str) -> bool:
    """True if the URL is an Instagram story with a specific story ID.

    yt-dlp on stories URLs has a known limitation: it fetches the user's
    *current* stories list and downloads the first one, ignoring the specific
    story ID in the URL. gallery-dl handles per-story IDs correctly. So we
    detect this URL pattern and route directly to gallery-dl.
    """
    return bool(_INSTAGRAM_STORY_RE.match(url.split("?", 1)[0]))
...
_BAD_PATH_CHARS = {
    "：": "_",  # fullwidth colon (Windows-compat substitution for `:`)
...
def _sanitize_path_component(name: str) -> str:
    """Replace unicode-substituted Windows-reserved chars with underscores.
    Downstream tools (Telegram bot, container path resolution) trip on these."""
    for bad, good in _BAD_PATH_CHARS.items():
        name = name.replace(bad, good)
    return name


def _sanitize_paths_in_tree(base_dir: str) -> None:
    """Walk base_dir bottom-up and rename any dir/file containing bad chars.
    Idempotent — safe to call multiple times."""
    base = Path(base_dir)
    # Walk DEEPEST first so renaming a parent doesn't break in-flight iteration
    for path in sorted(base.rglob("*"), key=lambda p: -len(p.parts)):
        clean = _sanitize_path_component(path.name)
        if clean != path.name:
            new_path = path.parent / clean
...
def _gallery_dl_run(base_dir: str, url: str, cookiepath: str | None) -> list[str]:
    ...
    Post-processes paths to strip Windows-compat unicode substitutions
    (e.g. fullwidth colon from `instagram:story` → `instagram_story`) that
    ...
    _sanitize_paths_in_tree(base_dir)
...
async def _run_download(
    ...
    async with _get_semaphore():
        ...
        # Instagram stories: yt-dlp ignores the per-story ID and grabs
        # whatever's first in the user's current stories list. gallery-dl
        # respects the specific story ID. Route accordingly.
        force_gallery_dl = _is_instagram_story(url)

        try:
            if media_type in ("photo", "carousel") or force_gallery_dl:
                # gallery-dl handles Instagram/TikTok images + stories natively
