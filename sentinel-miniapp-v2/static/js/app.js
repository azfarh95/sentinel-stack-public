...
(() => {
  ...
  function render(screen, data = {}) {
    ({
      ...
      'wd-tunnels': renderWdTunnels, 'wd-disk': renderWdDisk, 'wd-lm': renderWdLm,
  ...
  // Allowlists mirror watchdog.py MONITORED_CONTAINERS / MONITORED_PORTS /
  // HEALTH_ENDPOINTS. Keep in sync when adding services there.
  const _WD_DOCKER_NAMES  = new Set([
    'MetaMCP', 'Reminders MCP', 'SMDL MCP', 'Google WS MCP', 'Maps MCP',
    'Memory MCP', 'GitHub MCP', 'OneDrive MCP', 'SMDL (s.)',
    'Sentinel Finance', 'Vaultwarden', 'Firefly III', 'Firefly DB',
    'Firefly Importer', 'Headscale', 'LibreTranslate', 'MetaMCP Postgres',
    'PIA Exit (VPN)',
  ]);
  const _WD_PROC_SVC      = new Set(['Sentinel (OpenClaw)', 'LM Studio']);
  const _WD_PROC_EP       = new Set([
    'GitHub MCP :8091', 'Playwright proxy :8932', 'Infer Bridge',
    'Sentinel Bridge', 'Shopping MCP', 'Translate MCP', 'OpenClaw → MetaMCP',
    'OpenClaw Sidepanel Bridge', 'Comet-Sidepanel MCP',
  ]);
  const _WD_HTTP_NAMES    = new Set([
    'MetaMCP', 'Reminders MCP', 'SMDL MCP', 'Google WS MCP', 'Maps MCP',
    'Memory MCP', 'OneDrive MCP', 'Vaultwarden', 'SMDL (s.)',
    'Sentinel Finance', 'LibreTranslate', 'LM Studio API',
    'OpenClaw Sidepanel Bridge',
  ]);
  // Public-tunnel endpoints are distinguished by the "(public)" suffix in
  // their label. No allowlist needed — filter by name.includes("(public)").
    ...
    app.innerHTML = `
      <div class="page">
        ...
        <div class="card section-gap">
          ...
          <div class="btn-row section-gap">
            <button class="btn btn-secondary" id="nav-set" style="width:100%">⚙️ Settings</button>
          </div>
        </div>
      </div>`;
  ...
  function _applyHomeSnap(snap) {
    ...
    const tunnels = endpoints.filter(e => /\(public\)/i.test(e.name || ''));
    const disk    = Array.isArray(snap.disk)     ? snap.disk     : [];
    const lmList  = (snap.lm_model || status.lm_model || '').split(',')
                    .map(s => s.trim()).filter(Boolean);
    _wdData = { docker, procs, http, tunnels, disk, lm: lmList };

    const dockerUp    = docker.filter(i => i.ok).length;
    const procsUp     = procs.filter(i => i.ok).length;
    const httpUp      = http.filter(i => i.ok).length;
    const tunnelsUp   = tunnels.filter(i => i.ok).length;
    const dockerColor = _wdColor(docker);
    const procsColor  = _wdColor(procs);
    const httpColor   = _wdColor(http);
    const tunnelsColor = _wdColor(tunnels);
    // Disk: red if any pct >= 90, orange >= 75, green otherwise
    const diskMaxPct  = disk.reduce((m, d) => Math.max(m, d.pct || 0), 0);
    const diskColor   = diskMaxPct >= 90 ? '#ff3b30' : (diskMaxPct >= 75 ? '#ff9500' : '#4cd964');
    ...
    el.innerHTML = `
      ...
      <div class="card section-gap">
        ...
        <div class="wd-grid">
          ...
          <div class="wd-card" id="wd-tunnels">
            <div class="wd-card-header">
              <span class="wd-card-icon">🛰</span>
              <span class="wd-card-label">Tunnels</span>
              <div class="wd-card-dot" style="background:${tunnelsColor}"></div>
            </div>
            <div class="wd-card-count">${tunnelsUp}/${tunnels.length} <span class="wd-card-sub">up</span></div>
          </div>
          <div class="wd-card" id="wd-disk">
            <div class="wd-card-header">
              <span class="wd-card-icon">💾</span>
              <span class="wd-card-label">Disk</span>
              <div class="wd-card-dot" style="background:${diskColor}"></div>
            </div>
            <div class="wd-card-count">${diskMaxPct}% <span class="wd-card-sub">peak</span></div>
          </div>
          <div class="wd-card" id="wd-lm">
            <div class="wd-card-header">
              <span class="wd-card-icon">🧠</span>
              <span class="wd-card-label">LM Studio</span>
              <div class="wd-card-dot" style="background:${lmList.length ? '#4cd964' : 'var(--tg-theme-hint-color)'}"></div>
            </div>
            <div class="wd-card-count">${lmList.length} <span class="wd-card-sub">model${lmList.length===1?'':'s'} loaded</span></div>
          </div>
    ...
    document.getElementById('wd-tunnels').onclick = () => { haptic(); push('wd-tunnels'); };
    document.getElementById('wd-disk').onclick    = () => { haptic(); push('wd-disk'); };
    document.getElementById('wd-lm').onclick      = () => { haptic(); push('wd-lm'); };
  ...
  // ── Watchdog Control ──────────────────────────────────────────────────────
  // Shows the watchdog's known services with a Restart button per service +
  // a per-container Logs viewer. Backed by watchdog HTTP endpoints
  // (/restart and /logs) proxied through bridge.py /api/services/*.
  ...
  async function renderWatchdogControl() {
    app.innerHTML = `
      <div class="page">
        ...
        <div class="card section-gap" style="font-size:12px;color:var(--tg-theme-hint-color);line-height:1.4">
          ⓘ Restart spins the service back up via the watchdog's auto-restart
          path. Logs opens a per-container tail (docker services only).
        </div>
    ...
    try {
      ...
      if (!services.length) {
        body.innerHTML = '<div class="card"><div style="padding:12px;color:var(--tg-theme-hint-color)">No services reported by watchdog.</div></div>';
        return;
      }
      body.innerHTML = `<div class="card section-gap">${services.map(s => {
        const ok = !!s.healthy;
        const dot = ok ? 'active' : 'inactive';
        const safe_name = s.name.replace(/[^A-Za-z0-9_-]/g, '_');
        // We only know whether it has a docker container name from the label
        // mapping — best-effort: lowercase + spaces->hyphens
        const guess_container = s.name.toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-|-$/g, '');
        return `
          <div class="settings-row" style="align-items:center;padding:10px 4px">
            <div style="flex:1;min-width:0">
              <div class="status-row" style="margin-bottom:2px">
                <div class="status-dot ${dot}"></div>
                <div class="status-value">${esc(s.name)}</div>
              </div>
              <div style="font-size:11px;color:var(--tg-theme-hint-color);margin-left:18px">port ${s.port || '—'}</div>
            </div>
            <div style="display:flex;gap:6px;flex-shrink:0">
              <button class="btn btn-ghost wc-logs-btn" data-container="${esc(guess_container)}" data-label="${esc(s.name)}" style="width:auto;padding:4px 10px;font-size:12px">📋 Logs</button>
              <button class="btn btn-secondary wc-restart-btn" data-label="${esc(s.name)}" style="width:auto;padding:4px 10px;font-size:12px">↺ Restart</button>
            </div>
          </div>`;
      }).join('')}</div>`;

      body.querySelectorAll('.wc-restart-btn').forEach(btn => {
        btn.onclick = async () => {
          ...
          btn.textContent = '…'; btn.disabled = true;
          try {
            const r = await api('/api/services/restart', {
              method: 'POST',
              body: JSON.stringify({ label }),
            });
            toast(r.ok ? `${label}: restarted` : `${label}: failed`);
          } catch (e) {
            toast(`Restart failed: ${String(e).slice(0, 60)}`);
          } finally {
            btn.textContent = '↺ Restart'; btn.disabled = false;
            setTimeout(renderWatchdogControl, 2500);  // refresh status
          }
        };
      });

      body.querySelectorAll('.wc-logs-btn').forEach(btn => {
        btn.onclick = () => {
          haptic();
          push('watchdog-logs', {
      ...
      document.getElementById('wc-body').innerHTML =
        `<div class="card"><div style="padding:12px;color:#ff3b30">Failed to load services: ${esc(String(e))}</div></div>`;
    }
  }
